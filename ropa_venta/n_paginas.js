...
function aplicarFiltro(categoria) {
  ...
  productos.forEach(producto => {
    ...
    const visible = (categoria === 'todos' || cat.includes(categoria));
    producto.style.display = 'none'; // Ocultamos todo inicialmente

    if (visible) productosFiltrados.push(producto);
  });

  generarBotonesPaginacion();
  mostrarPagina(1); // Reiniciamos siempre en la primera página
}

// Generar botones según cantidad de productos filtrados
function generarBotonesPaginacion() {
  const contenedor = document.querySelector('.paginacion'); // Asegúrate de tener este contenedor en HTML
